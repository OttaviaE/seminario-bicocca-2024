...
data <- data.frame(
  item = c("", "1", "2", "\\(\\vdots\\)", "\\(i\\)", "\\(\\vdots\\)", "\\(I\\)"),
  col1 = c("", "\\(\\mathit{iif}_{11}\\)", "\\(\\mathit{iif}_{21}\\)", "", "\\(\\ldots\\)", "", ""),
  col2 = c("", "\\(iif_{12}\\)", "\\(\\mathit{iif}_{22}\\)", "", "\\(\\ldots\\)", "", ""),
  col3 = c("", "", "", "", "\\(\\ldots\\)", "", ""),
  col4 = c("", "\\(\\vdots\\)", "\\(\\vdots\\)", "\\(\\vdots\\)", "\\(\\mathit{iif}_{jn}\\)", "\\(\\vdots\\)", "\\(\\vdots\\)"),
  col5 = c("", "", "", "", "\\(\\ldots\\)", "", ""),
  col6 = c("", "", "", "", "\\(\\ldots\\)", "", ""),
  col7 = c("", "", "", "", "\\(\\ldots\\)", "", "\\(\\mathit{iif}_{IN}\\)"),
...
1. Select $iif_{jn}^k = \displaystyle  \max_{j \in J\setminus S^k, \, n  \in N \setminus Q^k} \mathbf{IIF}(j,n)$;
2. Compute $S^{k+1} = S^k \cup \{j\}$ as the set of item(s) selected at $k$; 
...
Diagnostic tests with cut-off values, tests for teh gited population
:::
...
```{r, echo=FALSE, fig.cap="TIF of the 10-item short test form"}
...
1. $\theta_{target} := \arg \max |\mathbf{TIF}^* - \mathbf{TIF}^{k}|$

::: {.fragment .highlight-red}
2. $i^* = \arg \min_{i \in B\setminus Q^{k}} |\theta_{target} - b_i|$
:::

3.  $\mathbf{pTIF}_{i^*} = \frac{TIF^k + IIF_{i^*}}{||Q^{k}|| + 1}$ 

4. Termination Criterion: $|\mathbf{TIF}^* - \mathbf{pTIF}_{i^*}| \geq |\mathbf{TIF}^* - \mathbf{TIF}^{k}|$: 
...
2. Random item selections of lengths $l$ from $B$ ($M_l = 5.01 \pm 2.99$) + randomly drawn values $\mathcal{U}(-0.20, 0.20)$ $\rightarrow$ $\mathbf{TIF}^*$ 
...
ggplot(all_ranks, 
  ...
  scale_color_hue(labels =c("Frank", "ILA", "ISA")) + 
  scale_shape(labels = c("Frank", "ILA", "ISA"))
